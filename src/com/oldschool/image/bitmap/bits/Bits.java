...
public enum Bits {
    ...
    BIT4(Constants.BITS_4) {
        @Override
        public BmpImage read(BmpHeader header, LitEndInputStream input, Pixel[] pixels) throws IOException {
            BmpImage outputImage = new BmpImage(header.getGrayscale() ? Constants.BITS_4_GRAY : Constants.BITS_4, header.getWidth(), header.getHeight());
            outputImage.setPixels(pixels);
            ...
            outputImage.setIndexColorModel(new IndexColorModel(4, header.getNumColors(), r, g, b));
            ...
            for (int y = header.getHeight() - 1; y >= 0; y--) {
                ...
                for (int x = 0; x < header.getWidth(); x++) {
                    ...
                    outputImage.setValue(index, x, y);
                }
            }
    ...
    BIT8(Constants.BITS_8) {
        @Override
        public BmpImage read(BmpHeader header, LitEndInputStream input, Pixel[] pixels) throws IOException {
            BmpImage outputImage = new BmpImage(header.getGrayscale() ? Constants.BITS_8_GRAY : Constants.BITS_8, header.getWidth(), header.getHeight());
            outputImage.setPixels(pixels);
    ...
    BIT32(Constants.BITS_32) {
        @Override
        public BmpImage read(BmpHeader header, LitEndInputStream input, Pixel[] pixels) throws IOException {
            BmpImage outputImage = new BmpImage(Constants.BITS_32, header.getWidth(), header.getHeight());
        ...
        public void write(BmpFile file, LitEndOutputStream input) throws IOException {
            ...
            for (int y = height - 1; y >= 0; y--) {
                for (int x = 0; x < width; x++) {
                    ...
                    int g = file.getImage().getGreen(x, y);
                    int b = file.getImage().getBlue(x, y);
                    int a = file.getImage().getAlpha(x, y);
